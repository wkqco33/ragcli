...
- **설정 로딩 (계층적 로드)**:
  - 설정 로드 순서: **CWD (`./config.yaml` / `./.env`)** → **전역 설정 (`~/.config/ragcli/config.yaml` / `.env`)**
  - CWD의 설정값이 전역 설정을 덮어씁니다 (Override).
