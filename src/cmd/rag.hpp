...
class RagCommand : public CommandBase {
  public:
    void register_to(wcppcli::Command &root) override {
        ...
        add_string_flag(*cmd, "model", 'm', "LLM model name (default: llama3)", &model_);
        add_string_flag(*cmd, "embed-model", 'e',
                        "Embedding model name (default: nomic-embed-text)", &embed_model_);
        add_string_flag(*cmd, "llm-url", 0, "LLM/Ollama base URL (default: http://localhost:11434)",
                        &llm_url_);
  ...
  private:
    auto run_rag() -> int {
        ...
        rag::RagCliOverrides overrides{llm_url_, model_, embed_model_, qdrant_url_, collection_};
        rag::RagTargets targets = rag::RagRunner::resolve_targets(overrides, conf);

        auto llm_client =
            llm_client::LLMClientFactory::create("ollama", /*api_key=*/"", targets.llm_url);
        ...
        if (chunker_type_ == "markdown") {
            ...
            chunker = std::make_shared<chunking::SimpleChunker>(512, 64);
        }
        ...
        if (!image_path_.empty()) {
            return run_index(document::create_source_from_path(image_path_), targets, llm_port,
                             qdrant_port, std::make_shared<chunking::NoChunker>());
        }
    ...
    auto run_index(std::shared_ptr<document::DocumentSource> source, const rag::RagTargets &targets,
                   std::shared_ptr<rag::LlmPort> llm_port,
                   std::shared_ptr<rag::QdrantPort> qdrant_port,
                   std::shared_ptr<chunking::Chunker> chunker =
                       std::make_shared<chunking::SimpleChunker>(512, 64)) -> int {
        auto embed_provider = std::make_shared<embedding::LlmEmbeddingProvider>(llm_port);
        ...
        indexing::Indexer indexer(embed_provider, qdrant_port, chunker);
